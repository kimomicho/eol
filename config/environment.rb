...
Rails::Initializer.run do |config|
  ...
  $PRODUCTION_MODE = true if ENV['RAILS_ENV'] and
    ['production', 'staging', 'siproduction', 'failover', 'preview','v2demo','v2prelaunch'].include?(ENV['RAILS_ENV'].downcase)
...
if defined?(PhusionPassenger)
  PhusionPassenger.on_event(:starting_worker_process) do |forked|
    if forked # We're in smart spawning mode.
      ...
      puts "*" * 120
      puts "Conservative spawn; no clone.  Cache nil? #{$CACHE.nil?}"
